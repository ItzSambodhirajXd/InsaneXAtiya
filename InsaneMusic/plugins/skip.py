...
from InsaneMusic.utils.inline.play import (stream_markup,
                                          telegram_markup,
                                          close_keyboard)
...
async def skip(cli, message: Message, _, chat_id):
    if not len(message.command) < 2:
                        ...
                        for x in range(state):
                            ...
                            if not check:
                                try:
                                    ...
                                    await InsaneMusic.stop_stream(chat_id)
                                except:
                                    return
                                break
                    else:
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await InsaneMusic.stop_stream(chat_id)
                except:
                    return
        except:
            try:
    ...
    if "live_" in queued:
        ...
        try:
            ...
            image = None
        try:
            await InsaneMusic.skip_stream(chat_id, link, video=status, image=image)
        except Exception:
        ...
        try:
            ...
            image = None
        try:
            await InsaneMusic.skip_stream(chat_id, file_path, video=status, image=image)
        except Exception:
            return await mystic.edit_text(_["call_9"])
        ...
        try:
            await InsaneMusic.skip_stream(chat_id, videoid, video=status)
        except Exception:
            return await message.reply_text(_["call_9"])
        button = telegram_markup(_, chat_id)
        run = await message.reply_photo(
        ...
        if videoid == "telegram":
            ...
            try:
                ...
                image = None
        try:
            await InsaneMusic.skip_stream(chat_id, queued, video=status, image=image)
        except Exception:
            return await message.reply_text(_["call_9"])
        if videoid == "telegram":
            button = telegram_markup(_, chat_id)
